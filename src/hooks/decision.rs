...
//! [`ApprovalOwner`], which only ever relaxes an ask.
...
///
/// This only ever relaxes an ask. It is applied by [`ApprovalOwner::apply`],
/// which matches on [`HookDecision::AskRewrite`] alone, so a [`HookDecision::Deny`]
/// and a [`HookDecision::Defer`] are structurally out of its reach — a host
/// name cannot turn a denied command into an allowed rewrite.
...
impl ApprovalOwner {
    ...
    /// Relax an ask into an allow when the delegate owns approval.
    ///
    /// Every other decision passes through by construction.
    pub(crate) fn apply(self, decision: HookDecision) -> HookDecision {
        match (self, decision) {
            (Self::Delegate, HookDecision::AskRewrite(rewritten)) => {
...
pub(crate) enum AgentPath {
    ...
    /// itself does not want RTK to ask as well.
...
impl AgentPath {
    ...
    /// relaxing an ask the agent would only ask about twice.
    pub(crate) fn decide(&self, cmd: &str) -> HookDecision {
        self.approval_owner()
            .apply(decide_for_agent(cmd, self.verdict(cmd)))
    }
}
...
mod tests {
    ...
    /// The load-bearing property of [`ApprovalOwner`]: it relaxes an ask and
    /// touches nothing else. A deny reaching `AllowRewrite` here is the
    /// failure the #3909 review found, so it is asserted directly rather than
    /// left to the exit-code layer.
    #[test]
    fn a_delegate_owning_approval_relaxes_an_ask_and_nothing_else() {
        let rewritten = || "rtk git status".to_string();
        assert_eq!(
            ApprovalOwner::Delegate.apply(HookDecision::AskRewrite(rewritten())),
            HookDecision::AllowRewrite(rewritten())
        );
        assert_eq!(
            ApprovalOwner::Delegate.apply(HookDecision::Deny),
            HookDecision::Deny
        );
        assert_eq!(
            ApprovalOwner::Delegate.apply(HookDecision::Defer),
            HookDecision::Defer
        );
        assert_eq!(
            ApprovalOwner::Delegate.apply(HookDecision::AllowRewrite(rewritten())),
            HookDecision::AllowRewrite(rewritten())
        );
    }
    ...
    fn rtk_owning_approval_changes_no_decision() {
        for decision in [
            ...
            assert_eq!(ApprovalOwner::Rtk.apply(decision), expected);
        }
    }
